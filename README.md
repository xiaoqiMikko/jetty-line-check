...
判定表由 `tools/gen_table.py` 从**五条 GitHub advisory 的 `vulnerabilities[]` + Maven Central 真 jar HEAD 探测**生成,
...
🔴 **只有主打的 2332 的受影响面站得住**(走私路径落在 `ServerConnector` / `HttpParser` 这条默认解析主路径上,
...
- **主打的 CVE-2026-2332 是请求走私:** 要前面还有一个前端代理 / LB,且两端对同一请求解读不一致,
  才谈得上危害。裸跑的 Jetty 上讲不出攻击故事 —— 这条 CVSS 是 **7.4**,不是「谁装了谁完」。
